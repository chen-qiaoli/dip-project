...
仓库中已经包含生成后的 YOLO 格式数据集和 YOLOv5s 预训练权重，因此老师 clone 后可以直接运行训练、检测和测试集评估。

## 未放入仓库的内容

以下文件不是运行脚本所必需，未放入 GitHub：

- 项目报告 PDF 和 LaTeX 源文件；
- 已训练输出目录 `runs_train/`、`runs_detect/`、`runs_val/`；
- 报告图片素材 `report_assets/`；
- 本地依赖缓存 `py_pkgs/`；
- PDF 预览图、中间标注预处理结果和临时缓存。
...
数据整理脚本会为每张图片生成一个覆盖主体食物区域的 YOLO 格式中心框。由于原始照片大多数是单份餐食居中拍摄，这种标注方式可以完成课程实验流程；但它不是逐张精细手工框选的标注。
